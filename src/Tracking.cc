    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                //但我觉得它好像也没有replace 只是告知替换而已
                if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                {
                    bOK = TrackReferenceKeyFrame();
		   // countRef++;
		   // cout<<"can not use the Velocity and we ust keyframe it is the "<<countRef<<endl;
                }
                else
                {
                    bOK = TrackWithMotionModel();
		   // countVelo++;
		    //cout<<"we use the Velocity and it is the  "<<countVelo<<endl;
                    if(!bOK)
		    {
                        bOK = TrackReferenceKeyFrame();
			//countVeloBreak++;
		        cout<<"aaaaaaaaaaaaaaa the Velocity is not ok!!!aaaaaaaaa "<<endl;
			//just for test
		    }
                }
            }
...
//运动模型那里已经把上一帧更新了　就直接设置为上一帧作为初值　然后优化
...
//如果是单目　就返回　如果上一帧就是关键帧就返回　如果不是处于VO的情况　就返回
...
bool Tracking::TrackWithMotionModel()
{
    ...
    UpdateLastFrame();
//为什么需要更新　因为可能上一帧在局部建图线程里面已经被优化了　所以需要读取新数据　然后
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
		...
		//it是observations的指针　指向那个能观测到当前地图点的KF 然后keyframeCounter是个ＭＡＰ　它对应的这个KF键的位值就++ 
            }
            else
            {
    ...
    for(vector<KeyFrame*>::const_iterator itKF=mvpLocalKeyFrames.begin(), itEndKF=mvpLocalKeyFrames.end(); itKF!=itEndKF; itKF++)
    {
      ...
      //就是说　如果局部地图已经有超过８０帧　那就算了　不要那些边缘的ＫＦ了　但是挺奇怪的　
      //最起码也应该根据共视点多少排个序再加吧　不然加到一半后面的加不了了　很不公平啊
